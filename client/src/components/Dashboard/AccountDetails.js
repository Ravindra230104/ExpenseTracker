...
export default function AccountDetails() {
  ...
  const totalIncome = account?.transactions?.reduce((acc, transaction) => {
    return transaction?.transactionType === "Income" ? acc + transaction?.amount : acc;
  }, 0);
//   console.log("totalIncome:",totalIncome);
  
  // Calculate total expenses
  const totalExpenses = account?.transactions?.reduce((acc, transaction) => {
    return transaction?.transactionType === "Expenses" ? acc + transaction?.amount : acc;
  }, 0);
          ...
          <div className="bg-gray-50 pt-12 sm:pt-16">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
              <div className="mx-auto max-w-4xl text-center">
                ...
                Expense is the outflow of income, and wise spending is the key to financial harmony.
                </p>
              </div>
            </div>
